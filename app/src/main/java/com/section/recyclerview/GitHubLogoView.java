...
public class GitHubLogoView extends View {
    ...
    public GitHubLogoView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mDuration = 2000;
        mMask = BitmapFactory.decodeResource(getResources(), R.drawable.githublogo);
        mPaint = new Paint();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final float percent = (mDirection ? diff % mDuration : mDuration - (diff % mDuration)) / (float) mDuration;
        mPaint.setColor((Integer) ARGB_EVAL.evaluate(percent, Color.LTGRAY, Color.YELLOW));
